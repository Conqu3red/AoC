...
def is_sorted(grid: List[List[Optional[str]]]):
    return (
        grid[2][1] == grid[2][2] == grid[2][3] == grid[2][4] == "A"
        and grid[4][1] == grid[4][2] == grid[4][3] == grid[4][4] == "B"
        and grid[6][1] == grid[6][2] == grid[6][3] == grid[6][4] == "C"
        and grid[8][1] == grid[8][2] == grid[8][3] == grid[8][4] == "D"
    )
...
def accessible_locations(x: int, y: int, grid: List[List[Optional[str]]]):
    ...
    def all_below_area(x: int, y: int, t: str):
        for _v in grid[x][y + 1:]:
            if _v != t:
                return False
        return True
    ...
    if index_lookup[v] == x and all_below_area(x, y, v):
        return


    # BUG: This function is broken AAAAAAAAAAAAAAA
    # cell
    column = grid[nx]
    ny = 1
    #print(nx, column)
    while True:
        if ny + 1 < len(column) and column[ny + 1] is None:
            ny += 1
        else:
            break
    
    if ny < len(column):
        if all_below_area(nx, ny, v) and is_path_clear(x, y, nx, ny, grid):
            yield nx, ny
            return
...
class Grid(NamedTuple): # memoize hack
...
def get_shortest_sort_route(amphipods: List[List[str]]):
    ...
    #D#C#B#A#
    #D#B#A#C#
    grid[2] += [amphipods[0][0], "D", "D", amphipods[1][0]]
    grid[4] += [amphipods[0][1], "C", "B", amphipods[1][1]]
    grid[6] += [amphipods[0][2], "B", "A", amphipods[1][2]]
    grid[8] += [amphipods[0][3], "A", "C", amphipods[1][3]]

    RecursiveGame.try_sort(Grid(grid), cost=0) # sorting seems to be taking way too many steps, some possibilities missed?
    print(f"Part 2: {RecursiveGame.least_cost}")
